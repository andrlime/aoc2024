...
struct Part1 {
    ...
    static constexpr std::string NAME = "5a";
    static constexpr std::string LABEL = "Day 5, Part 1";
    ...
    static auto solve(const t& t) -> std::string {
        ...
        const int total = std::ranges::fold_left(
            t.orderings, 0, [&](int acc, const Ordering& ordering) {
                ...
                const size_t middle_index = ordering.elements.size() / 2;
                return acc + ordering.elements[middle_index];
            }
        );
